...
//file opening approval mechanism
static DECLARE_WAIT_QUEUE_HEAD(file_open_wait);
static DEFINE_SPINLOCK(file_open_approval_lock);
...
static ssize_t file_open_approval_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (!file_open_request_pending) {
        spin_unlock_irqrestore(&file_open_approval_lock, flags);
        return -EINVAL;
    }
    
    if (strncmp(buf, "yes", 3) == 0 || buf[0] == '1') {
        file_open_approval_status = 1;
        printk(KERN_INFO "File open approved: %s\n", pending_filename);
    } else {
        file_open_approval_status = -1;
        printk(KERN_INFO "File open denied: %s\n", pending_filename);
    }
    
    spin_unlock_irqrestore(&file_open_approval_lock, flags);
    wake_up_interruptible(&file_open_wait);
    
    return count;
}

static int file_open_approval_show(struct seq_file *m, void *v)
{
    unsigned long flags;
    ...
    if (file_open_request_pending) {
        seq_printf(m, "FILE OPEN PENDING: %s\n", pending_filename);
        seq_printf(m, "Write 'yes' to approve or 'no' to deny\n");
    } else {
        seq_printf(m, "No file open pending\n");
    }
...
static int file_open_approval_open(struct inode *inode, struct file *file)
{
    return single_open(file, file_open_approval_show, NULL);
}

static const struct proc_ops file_open_approval_ops = {
    .proc_open = file_open_approval_open,
    .proc_read = seq_read,
    .proc_write = file_open_approval_write,
...
fs_initcall(file_open_approval_init);

static int wait_for_file_open_approval(const char *filename)
{
    unsigned long flags;
    int ret;
    
    spin_lock_irqsave(&file_open_approval_lock, flags);
    file_open_request_pending = 1;
    ...
    printk(KERN_ALERT "FILE OPEN REQUESTED: %s by %s (PID %d)\n",
           filename, current->comm, current->pid);
    printk(KERN_ALERT "Write to /proc/file_open_approval to approve\n");
    ...
    ret = (file_open_approval_status == 1) ? 0 : -EPERM;
    spin_unlock_irqrestore(&file_open_approval_lock, flags);
    
    return ret;
}

//file opening approval - END
			   ...
			   struct open_how *how)
{
    ...
    if (user_path_at(dfd, filename, LOOKUP_FOLLOW, &resolved_path) == 0) {

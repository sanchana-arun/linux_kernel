...
// //changing syscall for user approval START - sanchana arun
// static char approval_path[] = "/sbin/reboot_confirm";

// static int request_user_approval(void){
// 	char* argv[] = {approval_path, NULL};
...
// 	pr_info("reboot: Requesting user approval via %s\n", approval_path);

// 	ret = call_usermodehelper(argv[0], argv, envp, UMH_WAIT_PROC);

// 	if (ret != 0) {
//         pr_warn("reboot: User approval failed or denied (ret=%d)\n", ret);
//         return -EPERM;
//     }

// 	return 0;
// }
// //changing syscall for user approval END - sanchana arun
...
// //ATTEMPT 2
// //Sanchana: Global variable to hold the lock state - 1 = Locked and 0 = Unlocked START
// static int reboot_locked = 1;

// //this function is called when you read the file
// static ssize_t reboot_locked_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf){
// 	return sprintf(buf, "%d\n", reboot_locked);
// }


// //this function is called when you write to the file
// static ssize_t reboot_locked_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count){
// 	int val;
// 	int ret;
...
// //defining the file attribute with read/write permissions
// static struct kobj_attribute reboot_locked_attribute = __ATTR(reboot_locked, 0644, reboot_locked_show, reboot_locked_store);


// //initialize the sysfs file on boot
// static int __init reboot_lock_init(void)
// {
//     int error;

//     error = sysfs_create_file(kernel_kobj, &reboot_locked_attribute.attr);
//     if (error)
//         pr_debug("failed to create the reboot_locked sysfs file\n");
//     return error;
// }
// subsys_initcall(reboot_lock_init);
		...
		void __user *, arg)
{
	...
	// //changing syscall by sanchana - START

	// if(cmd == LINUX_REBOOT_CMD_RESTART || 
    //     cmd == LINUX_REBOOT_CMD_HALT || 
    //     cmd == LINUX_REBOOT_CMD_POWER_OFF ||
    //     cmd == LINUX_REBOOT_CMD_RESTART2){
	// 	if (request_user_approval() != 0) {
	// 		return -EPERM;
    //     }
	// }

	// //changing syscall by sanchana - END


	// //Sanchana: check if reboot is locked START
	// if(cmd == LINUX_REBOOT_CMD_RESTART ||
    //     cmd == LINUX_REBOOT_CMD_HALT ||
    //     cmd == LINUX_REBOOT_CMD_POWER_OFF ||
    //     cmd == LINUX_REBOOT_CMD_RESTART2){
	...
	// 		if(reboot_locked == 1){
	// 			pr_info("reboor: denied. system is locked. write 0 to /sys/kernel/reboot_locked to unlock.\n");
	// 		}
	// 	}
